 ...
 * 		+ { (funDef.visibility !== null)? '''«funDef.visibility.type» ''' : ""}
 * 		+ { (funDef.payable)? '''payable ''' : ""}
 * 		+ { (funDef.returnType !== null)? '''returns («funDef.returnType.compileTypes»)''' : ""}
 * 		+ '''{
 ...
 * return {
 * 	'''
 * 	«IF fb.dec!== null»«fb.dec.compileDeclaration»«ENDIF»
 * 	«IF fb.loop!== null»«fb.loop.compileLoopStructure»«ENDIF»
 * 	«IF fb.def!== null»«fb.def.compileDefinitions»;«ENDIF»
 * 	«IF fb.call!== null»«fb.call.compileFunctionCall»;«ENDIF»
 * 	'''
 * }
 * 
 * }
 * def compileEnum(EnumDeclaration en){
 * 	return '''enum '''
 * 		+ { (en.visibility !== null)? '''«en.visibility.type» ''' : ""}
 * 		+ '''«en.name» {«FOR e: en.variables»«e.name»«IF e != en.variables.last», «ENDIF»«ENDFOR»}'''
 * }
 * 
 * //type location visibility name[][] == ?
 * def compileArray(ArrayDeclaration arr){
 * 	return 
 * 	'''«IF arr.strucType !== null »«arr.strucType.name»«ELSE»«arr.type.compileTypes»«ENDIF»«FOR s: arr.sizes »[«s.value»]«ENDFOR»'''
 * 	+ { (arr.location !== null)?  ''' «arr.location.type»''' : "" }
 * 	+ { (arr.visibility !== null)?  ''' «arr.visibility.type»''' : "" }
 * 	+ { (arr.constant)?  "constant": "" }
 * 	+ ''' «arr.name»'''
 * 	+ {(arr.blocks !== null)? ''' = «arr.blocks.compileArrDefinition»''': "" }
 * 	+ {(arr.^var !== null)? ''' = «arr.^var.compileArrDefinition»''': "" }
 * 	+ {(arr.access !== null)? ''' = «arr.access.compileArrDefinition»''': "" }
 * 	}
 * 
 * 
 ...
 * 	[«IF set.values !== null»«FOR value: set.values SEPARATOR ','»«value.compileLogicalOperations»«ENDFOR»«ENDIF»]
 * 	«ENDFOR»'''	
 * }
 * 
 ...
 * 		MappingAccess: '''«e.map.compileDeclarationReference»[«e.index.compileLogicalOperations»]'''
 * 		ArrayAccess: '''«e.variable.compileDeclarationReference»«FOR el: e.indexes»[«el.value.compileLogicalOperations»]«ENDFOR»«IF e.field !== null».«e.field»«ENDIF»'''
 * 	}
 * }
 * 
 ...
 * }
 * 
 * def compileStruct(StructDeclaration st){
 * 	return {
 * 		'''struct'''
 * 		+ { (st.visibility !== null)?  ''' «st.visibility.type»''' : "" }
 * 		+ { (st.name !== null)?  ''' «st.name»''' : "" }
 * 		+ {'''{
 * 		«FOR f: st.fields SEPARATOR '\n'»«f.compileDeclaration»«ENDFOR»
 * 		}'''}
 * 		}
 * }
 * 
 ...
 * 	return { (mp.visibility !== null)?  ''' «mp.visibility.type»''' : "" }
 * 	+'''«mp.unnamedMappingDeclaration.compileUnnamedMappingDeclaration» «IF mp.location !== null »storage «ENDIF»«mp.name»'''
 * }

 * def String compileUnnamedMappingDeclaration(UnnamedMappingDeclaration dec){
 * 	return 
 * 	'''mapping («dec.type.compileTypes» =>'''
 * 	+ { (dec.secondRef !== null)? '''«dec.secondRef.compileDeclarationReference»''':""}
 * 	+ { (dec.second !== null)? '''«IF dec.second instanceof UnnamedMappingDeclaration» «UnnamedMappingDeclaration.cast(dec.second).compileUnnamedMappingDeclaration» «»«ELSE» «dec.second.compileTypes»«ENDIF»''': ""}
 * 	+ '''«IF (dec.array)»[]«ENDIF»)'''
 * }
 * 
 ...
 * 			'''«pr.ref.name»«IF pr.value !== null» «pr.operator.value» «LogicalOperations.cast(pr.value).compileLogicalOperations» «ENDIF»'''
 * 		}
 * 		else{ //inline declaration
 * 			{ (pr instanceof ArrayableDeclaration)? '''«pr.type.compileTypes»''': "" }
 * 			+ { (pr instanceof NonArrayableDeclaration) ? ('''«pr.type.compileTypes»''' + ((pr.location !== null)? ''' «pr.location.type»''' : "")) : "" }
 * 			+ { (PrimaryTypeDeclaration.cast(pr).visibility !== null)?  ''' «PrimaryTypeDeclaration.cast(pr).visibility.type»''' : "" }
 * 			+ { (PrimaryTypeDeclaration.cast(pr).name !== null)?  ''' «PrimaryTypeDeclaration.cast(pr).name»''' : "" }
 * 			+ { (pr.value !== null)? ''' «pr.operator.value» «LogicalOperations.cast(pr.value).compileLogicalOperations»''': ""}
 * 		}
 * 	}
 * }
 * 
 ...
 * 	return
 * 	'''«FOR el: str.blocks»«el.compileInternalBlock»«ENDFOR»'''
 * }
 * 
 * def compileIfWhileStructure(IfWhileStructure str) {
 * 	return
 * 	'''«str.type»(«str.condition.compileLogicalOperations»)''' 
 * 	+''' {
 ...
 * 	return
 * 	'''«str.type»(«str.initial.compileIntial» «str.condition.compileLogicalOperations»; «str.step.compileSingleDefinition»)'''
 * 	+''' {
 * 		«str.compileInternalLoop»
 * 	}
 * 	'''
 * }
 * 
 * def String compileLogicalOperations(LogicalOperations logic) {
 * 	return 
 * 	{(logic.negate)? '''!«logic.first.compileCondition» «FOR operation: logic.operations»«operation.compileConditionOperation»«ENDFOR»''':
 * 	(logic.ternary)? '''«logic.first.compileCondition» ? «logic.^true.compileCondition» : «logic.^false.compileCondition»''' :
 * 	'''«logic.first.compileCondition»«FOR operation: logic.operations» «operation.compileConditionOperation»«ENDFOR»'''
 * 	}
 * }
 * 
 * def compileCondition(Condition cond) {
 * 	return {(cond.ref !== null)? '''«cond.ref.compileDeclarationReference»''':
 * 			switch cond {
 * 					ArithmeticOperations: cond.compileArithmeticOperations
 * 					StringLiteral: '''"«cond.value»"'''
 * 					BooleanLiteral: cond.value
 * 					FunctionCall: cond.compileFunctionCall
 * 					ArrayAccess: cond.compileAccess
 * 					MappingAccess: cond.compileAccess
 * 					FieldAccess: cond.compileAccess
 * 					SpecialLiteral: '''«cond.name»'''
 * 					BlockhashFunction: cond.compileBlockhash
 * 					GasleftFunction: '''«cond.name»'''
 * 					MathematicalFunction: cond.compileMathematicalFunction
 * 					HashFunction: '''«cond.compileHashFunction»'''
 * 					EcrecoverFunction: cond.compileEcrecoverFunction
 * 				}
 * 			}
 * }
 * 
 ...
 * 	return
 * 	'''blockhash(«block.parameter.compileIntParameter»)'''
 * }
 * 
 * def compileMathematicalFunction(MathematicalFunction mat) {
 * 	return
 * 	'''«mat.function»(«FOR param: mat.parameters SEPARATOR ', '»«param.compileIntParameter»«ENDFOR»)'''
 * }
 * 
 * def compileHashFunction(HashFunction hash) {
 * 	return
 * 	'''«hash.name»(«hash.parameters.compileIntParameter»)'''
 * }
 * 
 * def compileEcrecoverFunction(EcrecoverFunction ecrecover) {
 * 	return
 * 	'''«ecrecover.function»(«FOR param: ecrecover.parameters SEPARATOR ', '»«param.compileIntParameter»«ENDFOR»)'''
 * }
 * 
 * def compileIntParameter(IntParameter param) {
 * 	return 
 * 	{(param.ref !== null)? '''«param.ref.compileDeclarationReference»''' : 
 * 	(param.param !== null)? '''«param.param.compileArithmeticOperations»''' :
 * 	(param.fun !== null)? '''«param.fun.compileFunctionCall»''' : ""
 * 	}
 * }
 * 
 ...
 * 	+{(op.seconds !== null)? '''«op.seconds.compileCondition»''' : ""}
 * }
 * 
 ...
 * 		LogicalOperations: primary.compileLogicalOperations
 * 	}
 * }
 * 
 ...
 * 	return
 * 	{
 * 		(num.hexValue !== null)? '''«num.hexValue.value»''' : 
 * 		(num.decimalValue !== null)? '''«num.decimalValue.value»''' : '''«num.intValue.value»'''
 * 	}
 * 	+ {(num.etherUnit !== null)? ''' «num.etherUnit.compileUnitTypes»''':""}
 * }
 * 
 * def compileUnitTypes(UnitTypes unit) {
 * 	return
 * 	{(unit.units !== null)? '''«unit.units.value»''' : 
 * 		(unit.time !== null)? '''«unit.time.value»''' : ""
 * 	}
 * }
 * 
 ...
 * 			+ '''«FOR index: array.indexes»[«index.value.compileLogicalOperations»]«ENDFOR»'''
 ...
 * 	return
 * 	'''«map.compileAccess» = «map.sec.compileLogicalOrFilled»'''
 * }
 * 
 * def compileSingleDefinition(SingleDefinition definition) {
 * 	return
 * 	{(definition.first !== null)? '''«definition.first»«definition.name.name»''':
 * 		(definition.second !== null)? '''«definition.name.name»«definition.second»''' : ""
 * 	}
 * }
 * 
 * def compileLogicalOrFilled(EObject obj){
 * 	return switch obj {
 * 		LogicalOperations: obj.compileLogicalOperations
 ...
 * 	return
 * 	'''«field.compileAccess» = «field.value.compileLogicalOrFilled»'''
 * }
 * 
 ...
 * 	return
 * 	'''«call.name.name» («IF call.parameters !== null»«FOR param: call.parameters SEPARATOR ', '»«param.compileLogicalOperations»«ENDFOR»«ENDIF»)'''
 * }
 */
}
