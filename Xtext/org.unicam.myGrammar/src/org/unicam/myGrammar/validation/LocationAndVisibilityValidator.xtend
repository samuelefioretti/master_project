...
class LocationAndVisibilityValidator extends AbstractOptGrammarValidator {
	...
	def noWrongLocation(LocationLiteral locationLiteral) {
		val current = locationLiteral.eContainer.eContainer.getRoot[eObj |
			eObj instanceof StructDeclaration
			|| eObj instanceof FunctionDefinition
		]
	...
	def checkDataLocation(ArrayDefinitionDeclaration inDec) {
		if(inDec.getRoot[obj | obj instanceof FunctionDefinition ] instanceof FunctionDefinition
			...
			error("Arrays must have a data location", inDec.asDeclaration, CustomSolidityPackage.Literals.ARRAY_DECLARATION__NAME)	
	}
	...
	def checkDataLocation(ConcreteStructDeclaration inDec) {
		if(inDec.getRoot[obj | obj instanceof FunctionDefinition ] instanceof FunctionDefinition
			...
			error("Structure must have a data location", inDec.asDeclaration, CustomSolidityPackage.Literals.CONCRETE_STRUCT_DECLARATION__NAME)	
	}
	...
	def checkDataLocation(NonArrayableDeclaration inDec) {
		if(inDec.getRoot[obj | obj instanceof FunctionDefinition ] instanceof FunctionDefinition
			&& inDec.location === null
			&& inDec.type.type.equals("string")
		)
			error("Strings must have a data location", inDec.asDeclaration, CustomSolidityPackage.Literals.NON_ARRAYABLE_DECLARATION__TYPE)	
	}
	...
	def storageLocation(MappingDeclaration map) {
		if(map.getRoot[e | e instanceof FunctionDefinition] instanceof FunctionDefinition) {
			...
			if(map.location !== null)
				error("Map Location must not be declared here", map, CustomSolidityPackage.Literals.MAPPING_DECLARATION__NAME );
		}
	}
}